...
* pequiv contains extended income information
merge 1:1 pid syear using "$IN\pequiv.dta", nogen keep(master matches)
* gripstr contains data from the grip strength test
merge 1:1 pid syear using "$IN\gripstr.dta", nogen keep(master matches)
* pwealth contains wealth information on the individual level
merge 1:1 pid syear using "$IN\pwealth.dta", nogen keep(master matches)
